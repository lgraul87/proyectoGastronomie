...
import vista.proveedor.MaterialProveedorView;
import vista.proveedor.ProductoProveedorView;
	...
	public static void start() {
		...
		do {
			...
			if (bOption == 1) {
				...
				if (!sUsuarioLogin.equals("Sin") && !sUsuarioLogin.equals("")) {
		...
		do {
			try {
				bOption = (byte) L.valida(""
						...
						//
						+ "\n  --Operaciones Lista de proveedor producto: 	(8)"
						//
						+ "\n  --Operaciones Lista de proveedor material: 	(9)"
						//
						+ "\n  --Salir: 					(10)"
						//
						+ "\n", 1, 10, 3);
			...
			if (bOption == 1) {
				...
				ProductoProveedorView.operacionesProductoProveedor(controlGeneral);
			}else if (bOption == 9) {
				MaterialProveedorView.operacionesMaterialProveedor(controlGeneral);
			}
		} while (errorControl);
	}
